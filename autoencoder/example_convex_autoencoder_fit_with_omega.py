...
if example == '2d':
    ...
    n1z, n2z = 5,5  # number of z neurons in convex function
    n1w, n2w = 5,5  # number of w neurons in convex function
...
n_convex = 6 # number of weight matrices in convex fcn
...
def convex_fcn(x,convex_params):
    W1,W2,W3,b1,b2,b3 = convex_params
    v=x[:,:dim_encoder]
    p=x[:,dim_encoder:]
    zw = act(W1 @ jnp.vstack((v.T,p.T)) + b1)
    zw = act(W2 @ jnp.vstack((zw, v.T, p.T)) + b2)
    y = W3 @ jnp.vstack((zw, v.T, p.T)) + b3
    return y.T
...
def init_fcn(seed):
    np.random.seed(seed)
    params_convex = [
        np.block([[np.random.randn(n1z,dim_encoder), np.random.randn(n1z,npar)],
                  [np.random.randn(n1w,dim_encoder), np.random.randn(n1w,npar)]]), # W1
        np.block([[np.random.rand(n2z,n1z), np.random.randn(n2z,n1w),
                   np.random.randn(n2z,dim_encoder), np.random.randn(n2z,npar)], # W2
                    [np.random.randn(n2w,n1z+n1w+dim_encoder+npar)]]),
        np.block([np.random.rand(ny,n2z), np.random.randn(ny,n2w+dim_encoder+npar)]), #W3                   
        np.random.randn(n1z+n1w, 1),  # b1
        np.random.randn(n2z+n2w, 1),  # b2
...
# y = W3 @ jnp.vstack((zw, v.T, p.T)) + b3
params_convex_min = [
    np.block([[-np.inf*np.ones((n1z,dim_encoder)), -np.inf*np.ones((n1z,npar))],
                [-np.inf*np.ones((n1w,dim_encoder)), -np.inf*np.ones((n1w,npar))]]), # W1
    np.block([[np.zeros((n2z,n1z)), -np.inf*np.ones((n2z,n1w)),
                -np.inf*np.ones((n2z,dim_encoder)), -np.inf*np.ones((n2z,npar))], # W2
                [-np.inf*np.ones((n2w,n1z+n1w+dim_encoder+npar))]]),
    np.block([np.zeros((ny,n2z)), -np.inf*np.ones((ny,n2w+dim_encoder+npar))]), #W3                   
    -np.inf*np.ones((n1z+n1w, 1)),  # b1
    -np.inf*np.ones((n2z+n2w, 1)),  # b2
    -np.inf*np.ones((ny, 1))  # b3
]
...
def create_convex_fcn(convex_params):
    W1, W2, W3, b1, b2, b3 = convex_params
    z1 = cp.logistic(W1[:n1z,:dim_encoder] @ v_cvx + W1[:n1z,dim_encoder:] @ p_cvx + b1[:n1z])
    w1 = cp.logistic(W1[n1z:,:dim_encoder] @ v_cvx + W1[n1z:,dim_encoder:] @ p_cvx + b1[n1z:])
    z2 = cp.logistic(W2[:n2z,:n1z] @ z1 + W2[:n2z,n1z:n1z+n1w] @ w1 + W2[:n2z,n1z+n1w:n1z+n1w+dim_encoder] @ v_cvx + W2[:n2z,-npar:] @ p_cvx + b2[:n2z])
    w2 = cp.logistic(W2[n2z:,:n1z] @ z1 + W2[n2z:,n1z:n1z+n1w] @ w1 + W2[n2z:,n1z+n1w:n1z+n1w+dim_encoder] @ v_cvx + W2[n2z:,-npar:] @ p_cvx + b2[n2z:])
    y = W3[:,:n2z] @ z2 + W3[:,n2z:n2z+n2w] @ w2 + W3[:,n2z+n2w:n2z+n2w+dim_encoder] @ v_cvx + W3[:,-npar:] @ p_cvx + b3
    return y
