...
M = 10000 # number of initial states
...
Tf = 25. + T0 # [K]
CAf_nominal = 10. # [kmol/m^3]
CA_ref = 8.0 # [kmol/m^3] reference to track
...
CAf_min=CAf_nominal-.5 # [kmol/m^3] 
CAf_max=CAf_nominal+.5 # [kmol/m^3]
...
umin = scale_T(umin)
umax = scale_T(umax)
CAf_min = scale_CA(CAf_min)
CAf_max = scale_CA(CAf_max)
CA_ref = scale_CA(CA_ref)
# ###################################
# Define stage cost functions

@jax.jit
def stage_cost_x(x,CA_ref):
    loss = (x[1]-CA_ref)**2 # cost computed on scaled variables
    return loss
...
def stage_cost_x_cvx(x,CA_ref):
    loss = cp.sum_squares(x[1]-CA_ref) # cost computed on scaled variables
    return loss
...
# Define dynamics
@jax.jit
def control_affine_dynamics(x, CAf):
    x=unscale_x(x)
    CAf = unscale_CA(CAf)
    T = x[0]     # [K]
    CA = x[1]    # [kg mol/m^3]
    ...
    #xdot = jnp.array([FoV * (Tf - T) - UAoV/rhocp * (T - T_c) + minusDeltaH/rhocp * r,
    #                    FoV * (CAf - CA) - r])
...
@jax.jit
def dynamics(x, inputs):    
    u = inputs.reshape(2,1)[0,1]
    CAf = inputs.reshape(2,1)[1,1]
    # T_c = u[0]  # manipulated input [K]
    f, g = control_affine_dynamics(x, CAf)
    xnext = scale_x(f + g*unscale_T(u))
    return xnext, xnext

@jax.jit
def simulation(x0, U, CAf):    
    _, X = jax.lax.scan(dynamics, jnp.array(x0), jnp.hstack((U, jnp.tile(CAf,(U.shape[0],1)))))
    return X # return state trajectory x(1),x(2),...,x(H)

@jax.jit
def ss_residual(xu, CA_ref, CAf):
    x = xu[:nx]
    u = xu[nx:]
    xnext,_ = dynamics(x, jnp.array([u, CAf]))
    return jnp.hstack((xnext-x, xnext[1]-CA_ref))

@jax.jit
def steady_state(CA_ref, CAf):
    ...
    xu = broyden.run(CA_ref=CA_ref, CAf = CAf, init_params=xu).params
    return xu[:nx], xu[nx:]

# # Test
xss,uss = steady_state(jnp.array([scale_CA(8.)]),jnp.array([scale_CA(CAf_nominal)]))
X=simulation(xss,jnp.tile(uss,(100,1)),scale_CA(CAf_nominal))
...
@jax.jit
def opt_control_loss(U, x0, CAf, CA_ref, Tj_ref):
    ...
    X = simulation(x0, U, CAf)
    #loss = stage_cost_x(x0,CA_ref)[0][0]
    loss = jnp.sum(discount*stage_cost_u_all(U,jnp.tile(Tj_ref,(H,1))).reshape(-1))
    loss += jnp.sum(discount*stage_cost_x_all(X,jnp.tile(CA_ref,(H,1))).reshape(-1))
    # loss += 1e3*(X[-1,1]-CA_ref)**2 # terminal cost
    return loss
...
if GenerateData:
    ...
    def solve_optimal_control_problem(seed):
        ...
        CAf = (CAf_max-CAf_min)*np.random.rand(1)+CAf_min # random measured disturbance signal
        xss, Tj_ref = steady_state(CA_ref, CAf)
        ...
        Uopt, status = solver.run(U_guess, bounds = (Umin,Umax), x0=x0, CAf=CAf, CA_ref=CA_ref, Tj_ref=Tj_ref)
        Xopt = simulation(x0,Uopt, CAf)
        loss = opt_control_loss(Uopt,x0, CAf, CA_ref, Tj_ref)
        loss = loss.item()
        failed = np.isnan(loss) or loss>200. # discard NaNs and outliers
        
        print(f"seed = {seed: 3d}"
              f", CA_ref = {unscale_CA(CA_ref): 5.4f}"
              f", CA(end) = {unscale_CA(Xopt[-1,1]): 5.4f}"
              f", CAf = {unscale_CA(CAf.item()): 5.4f}"
              f", cost = {loss: 5.4f}", end="") 
        if failed:
            print(" (FAILED!)")
        else:
            print(" (OK)")
        
        return {"CAf": CAf, "CA_ref": CA_ref, "Tj_ref": Tj_ref, "x0": x0, "loss": loss, "failed": failed, "Uopt": Uopt, "Xopt": Xopt}
                
    results = Parallel(n_jobs=10)(delayed(solve_optimal_control_problem)(seed) for seed in range(M))
    
    pickle.dump(results, open('optimal_control_samples.pkl', 'wb'))

else:
    results = pickle.load(open('optimal_control_samples.pkl', 'rb'))

CAF = [results[i]["CAf"] for i in range(M)]
...
Theta = np.hstack((np.vstack(CAF),np.vstack(TJ_REF))) # parameters
...
if TrainModel:
    ...
    data = {"X": X, "Theta": Theta, "F": F, "CA_REF": CA_REF, "TJ_REF": TJ_REF, "Fmax": Fmax, "stats": stats, "params": pcf.model.params}
    pickle.dump(data, open('optimal_control_fit.pkl', 'wb'))

    print(f"Elapsed time: {stats['time']} s")

else:
    data = pickle.load(open('optimal_control_fit.pkl', 'rb'))
...
CAf_cvx = cp.Parameter((1,1))
CA_ref_cvx = cp.Parameter((1,1))
Tj_ref_cvx = cp.Parameter((1,1))

cvx_loss = gamma*Fmax*pcf.tocvxpy(x1_cvx,cp.vstack((CAf_cvx,Tj_ref_cvx))) + stage_cost_u_cvx(u_cvx,Tj_ref_cvx) + stage_cost_x_cvx(x1_cvx,CA_ref_cvx)
...
if 0:
    ...
    X= [x0]
    U= list()
    loss_x0=stage_cost_x(x0.reshape(nx,1),CA_ref).item()

    loss=loss_x0
    gammak = 1.
    for k in range(H):
        CAf_cvx.value = CAf.reshape(1,1)
        CA_ref_cvx.value = CA_ref.reshape(1,1)
        Tj_ref_cvx.value = Tj_ref.reshape(1,1)
        fx,gx = control_affine_dynamics(xk.reshape(nx,1), CAf)
    ...
    for h in range(Nval):
        CAf= (CAf_max-CAf_min)*np.random.rand(1)+CAf_min # random measured disturbance signal
        _, Tj_ref  = steady_state(CA_ref, CAf)
        
        xmin = scale_x(np.array([Tc_min, CA_min]))
        xmax = scale_x(np.array([Tc_max, CA_max]))
        x0 = jnp.array([(xmax[0]-xmin[0])*np.random.rand()+xmin[0], (xmax[1]-xmin[1])*np.random.rand()+xmin[1]]) # random initial state

        CAf_cvx.value = np.array(CAf).reshape(1,1)
        CA_ref_cvx.value = np.array(CA_ref).reshape(1,1)
        ...
        for k in range(H):
            # ADP solution
            fx,gx = control_affine_dynamics(xk1.reshape(nx,1), CAf)
            f0_cvx.value = np.array(fx).reshape(nx,1)
            g0_cvx.value = np.array(gx).reshape(nx,1)
            cvx_prob.solve()
            uk1 = u_cvx.value
            ...
            Uopt, status = solver.run(Uopt, bounds = (Umin,Umax), x0=xk2.reshape(nx), CAf=CAf, CA_ref=CA_ref, Tj_ref=Tj_ref)
            uk2 = Uopt[0,:]
            fx,gx = control_affine_dynamics(xk2.reshape(nx,1), CAf)
        ...
        ax[h//2,h%2].plot(unscale_CA(np.vstack(X1)[:,1]),label='ADP')
        ax[h//2,h%2].plot(unscale_CA(np.vstack(X2)[:,1]),label='Optimal')
        ax[h//2,h%2].plot(np.ones(H)*unscale_CA(CA_ref),label='Reference')
        ax[h//2,h%2].set_title("Concentration [kmol/m^3]")
        ax[h//2,h%2].legend()
        ax[h//2,h%2].grid()
        print(" ")
